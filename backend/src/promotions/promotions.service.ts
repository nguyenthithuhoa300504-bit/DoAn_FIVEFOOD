...
export class PromotionsService {
  ...
  async createPromotion(data: any) {
    ...
    const result = await this.dbService.query(query, [
      ...
      { name: 'DiscountPercentage', type: sql.Decimal(5, 2), value: DiscountPercentage },
      { name: 'MaxDiscountAmount', type: sql.Decimal(18, 2), value: MaxDiscountAmount || null },
      { name: 'MinOrderValue', type: sql.Decimal(18, 2), value: MinOrderValue || 0 },
    ...
    return { success: true, message: 'Tạo mã giảm giá thành công', promotionId: result.recordset[0].PromotionID };
  }
  ...
  async updatePromotion(id: number, data: any) {
    ...
    if (checkResult.recordset.length > 0) {
      throw new BadRequestException('Mã giảm giá này đã tồn tại ở một chương trình khác.');
    }
  ...
  async deletePromotion(id: number) {
    ...
    if (checkOrder.recordset.length > 0) {
      throw new BadRequestException('Không thể xóa mã giảm giá này vì đã có đơn hàng sử dụng.');
    }
