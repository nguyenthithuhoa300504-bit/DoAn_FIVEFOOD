...
*   **Thiết kế & Giao diện**: TailwindCSS, sử dụng font chữ hiện đại (Google Fonts như Inter/Roboto), hiệu ứng Glassmorphism và chuyển động mượt mà (micro-animations).
...
*   **Kết nối Realtime**: **Socket.io Client** để nhận thông báo và tọa độ shipper thời gian thực.
...
*   **Tích hợp AI**: Thư viện SDK chính thức của Google `@google/generative-ai` để tương tác trực tiếp với mô hình **Gemini 1.5 Flash API** của Google AI Studio.
*   **Kết nối Database**: Gọi trực tiếp Stored Procedures/Triggers của SQL Server.
...
*   **Simulation**: Backend phát tọa độ shipper định kỳ (ví dụ: mỗi 5 giây) di chuyển dọc theo tuyến đường từ Cửa hàng đến Khách hàng, Frontend lắng nghe sự kiện từ WebSocket và cập nhật vị trí biểu tượng shipper trên bản đồ thời gian thực.
...
### B. Tích hợp AI Chatbot (Gemini API)
*   Backend gọi API của Google AI Studio (Gemini 1.5 Flash).
